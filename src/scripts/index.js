...
(async function () {
  ...
  const yearApplicantsFile = join("src", "data", `${academicYear}.json`);
  ...
  while (!done) {
    ...
    rows.each((_, el) => {
      ...
      if (name !== "" && programme !== "") {
        applicants.push({ name, programme });
      }
    });
    ...
    if (threshold === 0) {
      console.log("[!] Threshold reached. Sleeping for 2 mins ");

      threshold = 5;

      await new Promise((resolve) => {
        setTimeout(() => resolve(true), 60000 * 2);
      });
    }
  }
})();
